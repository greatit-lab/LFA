    ...
    def on_created(self, event):
        if not event.is_directory:
            ...
            if event_id not in self.processed_events or datetime.now() - self.processed_events[event_id] > self.debounce_time:
                ...
                self.log_debug(f"BaseDateFolderHandler: Created event detected for {normalized_path}")
            else:
    ...
    def on_deleted(self, event):
        if not event.is_directory:
            ...
            if event_id not in self.processed_events or datetime.now() - self.processed_events[event_id] > self.debounce_time:
                ...
                self.log_debug(f"BaseDateFolderHandler: Deleted event ignored for {normalized_path} due to debounce")
